...
export namespace Chats {
    export const useChatState = create<ChatState>((set, get: () => ChatState) => ({
        ...
        reset: () => {
            ...
            set({ id: undefined, ghost: false })
            // leaving a ghost chat erases it permanently
            if (ghost && previousId) purgeGhostChat(previousId)
        },
    ...
    export namespace db {
        export namespace query {
            ...
            /**
             * Newest entry of a chat with its active swipe, used by the continue button
             */
            export const chatLatestEntry = async (chatId: number) => {
                return await database.query.chatEntries.findFirst({
        ...
        export namespace live {
            /**
             * Chat row for live UI use: memory, background, active preset, hidden and ghost flags
             */
            export const chat = (chatId: number) => {
                return database.query.chats.findFirst({
                    where: eq(chats.id, chatId),
                    // scroll_offset and last_modified are left out so scrolling does not rerender
                    columns: {
                        id: true,
                        name: true,
                        hidden: true,
    ...
    export const importBackground = async (chatId: number, oldBackground?: number | null) => {
        try {
            ...
            Logger.infoToast(t('chat.background.set'))
        } catch (e) {
            Logger.errorToast(t('chat.background.setFailed'))
            Logger.error(`${e}`)
        }
    }
    ...
    export const removeBackground = async (chatId: number, imageId: number) => {
        try {
            await db.mutate.updateBackground(chatId, null)
            await Characters.deleteImage(imageId)
            Logger.infoToast(t('chat.background.removed'))
        } catch (e) {
            Logger.errorToast(t('chat.background.removeFailed'))
            Logger.error(`${e}`)
        }
    }
